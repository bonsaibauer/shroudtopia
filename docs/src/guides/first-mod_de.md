...
Eine Windows-x64-DLL erstellen, `api/include` zum Include-Pfad hinzufügen, `shroudtopia/api.h` einbinden und `CreateMod` exportieren.
...
static Result CALL Load(const Api* host, void*) {
    return host->log(View("example.hello"), LOG_INFO, View("Hallo von Shroudtopia"));
}
...
Spiel starten, die Owner-markierte Meldung in `shroudtopia.log` suchen und Loader-Fehler beheben, bevor Services ergänzt werden. Danach mit [Ownership](../concepts/ownership.md) und der [Host API](../reference/api.md) fortfahren.
