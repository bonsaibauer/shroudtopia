...
`Registration`, `RuntimePatch` und `TextWindow` sind opake, Owner-gebundene Handles. Einzelne Handles möglichst direkt freigeben und beim Unload `release_owner` als Sicherheitsnetz aufrufen. Ein Handle nie nach Release oder mit anderem Owner verwenden.
