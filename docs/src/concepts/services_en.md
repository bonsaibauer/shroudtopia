...
`register_service` and `find_service` remain only for optional contracts published by third-party Mods. They are not the route to Shroudtopia's built-in API domains. A provider owns an extension table until it unregisters; consumers must not retain it across provider unload.
