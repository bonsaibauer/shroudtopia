...
int main() {
    ...
    try {
        ...
        TextWindow window = 0;
        check(ui->create(owner, &descriptor, &window) == RESULT_OK && window != 0);
        TextWindowStatus status = TEXT_PENDING;
        for (int attempt = 0; attempt < 100 && status == TEXT_PENDING; ++attempt) {
            Sleep(10); check(ui->get_status(owner, window, &status) == RESULT_OK);
        }
        check(status == TEXT_READY);
        check(ui->set_text(stranger, window, 0, label) == RESULT_PERMISSION_DENIED);
        check(ui->destroy(stranger, window) == RESULT_PERMISSION_DENIED);
        check(ui->set_text(owner, window, 0, label) == RESULT_OK);
        check(ui->set_text(owner, window, 1, label) == RESULT_INVALID_ARGUMENT);
        check(ui->destroy(owner, window) == RESULT_OK);
        check(ui->get_status(owner, window, &status) == RESULT_NOT_FOUND);
        check(ui->create(owner, &descriptor, &window) == RESULT_OK);
