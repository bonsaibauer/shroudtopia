...
Result CALL ToggleFlight(StringView, void*) {
    ...
    const auto result = Runtime->set_patch_enabled(View(ModId), Patch, enable ? 1 : 0);
...
Result CALL Activate(const Api*, void*) {
    ...
    const auto result = Runtime->set_patch_enabled(View(ModId), Patch, 1);
...
Result CALL Deactivate(const Api*, void*) {
    ...
    const auto result = Runtime->set_patch_enabled(View(ModId), Patch, 0);
