...
namespace {
    ...
    Result CALL Load(const Api* api, void*) {
        ...
        const ModSettingsDescriptor mod_settings{
            sizeof(ModSettingsDescriptor), View("mod.commands.settings"),
            View("{\"type\":\"object\",\"additionalProperties\":false}"),
            View("{}")
        };
        return api->register_mod_settings(View(ModId), &mod_settings, &state.mod_settings);
    }
    ...
    Result CALL Unload(const Api* api, void*) {
        if (api == nullptr) return RESULT_INVALID_ARGUMENT;
        const Result result = api->release_owner(View(ModId));
        state = {};
        return result;
    }
}
